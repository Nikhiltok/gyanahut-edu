...
export async function sendEmailOtp() {
  const { data } = await api.post<ApiSuccess<{ debug_otp?: string }>>("/users/email/send-otp/");
  return data.data;
}

export async function verifyEmailOtp(code: string) {
  const { data } = await api.post<ApiSuccess<Profile>>("/users/email/verify/", { code });
  return data.data;
}
...
export async function login(payload: LoginPayload) {
  ...
  // The previously-logged-in user's cached queries (profile, dashboard stats, etc.)
  // must not leak into this new session — clear them all so every component
  // refetches fresh data for whoever just logged in.
...
export async function updateProfileAvatar(file: File) {
  ...
  // The shared axios instance defaults to "Content-Type: application/json" — that
  // default must be cleared for this one request so the browser can set its own
